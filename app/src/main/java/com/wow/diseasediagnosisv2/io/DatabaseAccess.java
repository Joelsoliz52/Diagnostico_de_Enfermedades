...
public class DatabaseAccess {
    ...
    /////////////////////////
    // Codigo Omar
    public ArrayList<String> getActualizaciones(){
        c = db.rawQuery("SELECT * FROM CONEXION_INTERNET WHERE ID_CONEXION = (SELECT MAX(ID_CONEXION) FROM CONEXION_INTERNET)",new String[]{});
        ArrayList<String> listaActualizaciones = new ArrayList<>();
        while(c.moveToNext()){
            String tmpUno = c.getString(0);
            String tmpDos = c.getString(1);
            String tmpTres = c.getString(2);
            String tmpCuatro = c.getString(3);
            listaActualizaciones.add(tmpUno);
            listaActualizaciones.add(tmpDos);
            listaActualizaciones.add(tmpTres);
            listaActualizaciones.add(tmpCuatro);
        }
